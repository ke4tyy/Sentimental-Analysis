...
ReviewList* Analysis::copyList(ReviewList& list) {
	ReviewList* newList = new ReviewList;

	if (list.head == nullptr) return;

	ReviewNode* current = list.head;
	while (current != nullptr) {
		newList->addReview(current->review, current->rating, current->goodCount,
			current->badCount, current->sentiment,
